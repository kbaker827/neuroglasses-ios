// GlassesStreamServer.swift — updated to use Rokid AI glasses SDK
...
// ─────────────────────────────────────────────────────────────────────────────
@MainActor
final class GlassesStreamServer: ObservableObject {
    ...
    func broadcastDone() {
        guard let dev = activeDevice else { return }
        RokidMobileSDK.vui.sendMessage(topic: "done", text: "", to: dev)
    }
...
extension GlassesStreamServer: SDKBinderObserver {
